...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h1 class="h4">Assigned Conversations</h1>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle">
                ...
                <?php if (!$conversations): ?>
                    <tr><td colspan="5" class="text-center text-muted">No conversations assigned.</td></tr>
                <?php else: ?>
                    <?php foreach ($conversations as $conversation): ?>
                        <tr>
                            <td><?php echo sanitize_input($conversation['subject']); ?></td>
                            <td>
                                <?php echo sanitize_input($conversation['student_name']); ?>
                                <div class="small text-muted"><?php echo sanitize_input($conversation['registration_number']); ?></div>
                            </td>
                            <td><?php echo conversation_status_badge($conversation['status']); ?></td>
                            <td><?php echo format_datetime($conversation['updated_at']); ?></td>
                            <td>
                                <a href="/staff/conversation.php?id=<?php echo (int) $conversation['id']; ?>" class="btn btn-sm btn-outline-primary">Open</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
